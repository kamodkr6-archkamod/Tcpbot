...
async def permanent_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "❤️ Start Like":
        ...
        keyboard = [
            [
                InlineKeyboardButton("🇮🇳 IND", callback_data="like_ind"),
                InlineKeyboardButton("🇧🇩 BD", callback_data="like_bd")
            ]
        ]
    ...
    # 📍 VISIT BUTTON (NO FUNCTION)
    if text == "📍 Send Visit Only":
        return
...
async def like_region_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):

    query = update.callback_query
    await query.answer()

    if query.data == "like_ind":
        region = "IND"
    else:
        region = "BD"
...
if __name__ == '__main__':
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, permanent_menu_handler))
    app.add_handler(CallbackQueryHandler(like_region_handler, pattern="like_"))
